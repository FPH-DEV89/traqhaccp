...
> ⓘ **État de l'installation** — la reconnaissance automatique est **active** : le serveur dispose de la clé d'analyse d'images requise (chapitre 18). Si le service d'analyse est momentanément indisponible (connexion coupée, service en ligne injoignable), l'appui sur **Prendre photo** affiche un message d'information précisant la cause et le formulaire s'ouvre **vide** : l'application **n'invente jamais** de lot ni de DLC, et la saisie manuelle (chapitre 7) produit le même registre. Le lien **Tester avec l'étiquette de démonstration** déroule tout le parcours sur une étiquette d'exemple ; les valeurs alors proposées sont des valeurs de démonstration, **à ne jamais conserver pour un lot réel**.
...
L'application vous a affiché un message d'information, puis ouvert le formulaire vide : c'est le comportement prévu lorsque l'analyse d'images n'est pas disponible — l'application ne remplit **jamais** le formulaire avec des valeurs inventées. Trois causes, dans l'ordre de fréquence : **(a)** le service d'analyse est momentanément injoignable (connexion coupée, service en ligne indisponible) — vérifiez la connexion et réessayez ; **(b)** la photo est inexploitable (floue, de biais, reflet, étiquette froissée) — recadrez la zone **lot + DLC** et recommencez ; **(c)** le navigateur n'a pas l'autorisation d'utiliser l'appareil photo. Dans les trois cas, la **saisie manuelle (chapitre 7) produit exactement le même registre** : dix secondes de plus, rien de moins.
