#!/usr/bin/env node
/**
 * check-norms-consistency.mjs — Cohérence des seuils sanitaires.
 *
 * Source de vérité : src/domain/haccp_norms.js (NORMS, ALLERGENS_14).
 * Comparée aux exports hérités v3 : src/domain/constants.js (HACCP_NORMS, ALL_14_ALLERGENS).
 *
 * Ne dépend PLUS de test_clean_arch.js (supprimé) ni de index.html (supprimé).
 * Les deux fichiers de domaine cités sont les SEULS encore vivants et importés
 * par l'app livrée (patisserie.html → js/patisserie → src/domain/).
 *
 * Vérifie que :
 *   1. toutes les valeurs que les deux jeux DOIVENT partager sont identiques (EGALITES) ;
 *   2. les écarts connus et justifiés restent exactement ceux documentés (DIVERGENCES) —
 *      toute harmonisation silencieuse doit être assumée ici et dans les commentaires.
 *
 ...
 * `relation(valeurV4, valeurV3)` doit rester vraie tant que le commentaire
 * qui justifie l'écart est en place.
 */
const DIVERGENCES = [
  {
    quoi: 'Huiles — seuil d\'alerte TPM',
    valeurV4: NORMS.oil.tpmAlert, valeurV3: O.WARNING_TPM,
  ...
  },
  {
    ...
    valeurV4: NORMS.weight.lossAlertPct, valeurV3: W.DEFAULT_TOLERANCE_PERCENT,
    relation: (v4, v3) => v4 > v3,
  ...
  },
  {
    ...
    valeurV4: NORMS.defrost.maxHours, valeurV3: D.MAX_DAYS * 24,
    relation: (v4, v3) => v4 < v3,
  ...
  },
  {
    ...
    valeurV4: NORMS.ph.max, valeurV3: HACCP_NORMS.PH.CANNED_ACID.max,
    relation: (v4, v3) => v4 > v3,
    pourquoi: 'v4 borne la saisie des aliments (pH 2 à 7) ; HACCP_NORMS.PH porte des valeurs cibles '
      + 'par famille (sushi ≤ 4,3 ; conserves < 4,5 ; marinades ≤ 4,2).',
  },
];
...
for (const { quoi, valeurV4, valeurV3, relation, pourquoi } of DIVERGENCES) {
  if (!relation(valeurV4, valeurV3)) {
    echecs.push(`${quoi} : écart documenté non respecté (v4=${valeurV4}, v3=${valeurV3}) — ${pourquoi}`);
  }
}
